...
## 🎯 Business Question
Which advertising campaign delivered a positive return on investment and actually increased the delivery service's profits?
...
total_revenue AS (SELECT c.ads_campaign,
       SUM(price) AS revenue
FROM campaigns c
    LEFT JOIN orders o
    ON c.order_id = o.order_id
    CROSS JOIN unnest(product_ids) AS g(product_id)
    LEFT JOIN products p 
    ON p.product_id = g.product_id
GROUP BY c.ads_campaign)

SELECT ads_campaign,
       ROUND(((revenue-250000)::decimal / 250000 * 100), 2) as ROI
FROM total_revenue
ORDER BY ROI DESC
```
...
## 📊 Analytical Insights
Despite the higher CAC, the first campaign has a positive ROI, meaning it recoups the advertising costs. Meanwhile, the second campaign has a negative ROI — it's currently operating at a loss. These figures may change over time, but for now, the first campaign is a more profitable investment. Let's look at other metrics.
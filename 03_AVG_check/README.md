...
WITH valid_orders AS (
    -- Заказы, которые не были отменены
    SELECT ua.user_id, ua.order_id
...
campaigns AS (
    -- Кампания №1
    SELECT DISTINCT vo.user_id, 'Campaign № 1' AS ads_campaign, vo.order_id
    ...
    -- Кампания №2
    SELECT DISTINCT vo.user_id, 'Campaign № 2' AS ads_campaign, vo.order_id
    FROM valid_orders vo
    WHERE vo.user_id IN (
...
order_check AS (SELECT c.user_id, c.ads_campaign, c.order_id, SUM(p.price) AS order_total
    FROM campaigns c
    LEFT JOIN orders o
        ON c.order_id = o.order_id
    CROSS JOIN UNNEST(o.product_ids) AS g(product_id)
    LEFT JOIN products p
        ON p.product_id = g.product_id
    WHERE o.creation_time >= '2022-09-01'
      AND o.creation_time <  '2022-09-08'
    GROUP BY c.user_id, c.ads_campaign, c.order_id),

user_avg AS (
    SELECT user_id, ads_campaign, AVG(order_total) AS avg_user_check
    FROM order_check
...
---
![AOV chart](AVG_check.png)

## 📊 Analytical Insights
For delivery services, sustainable growth is achieved not through one-time monetization, but through repeat orders. Therefore, when evaluating marketing channels, it's essential to analyze AOV in conjunction with retention and purchase frequency. Let's explore further